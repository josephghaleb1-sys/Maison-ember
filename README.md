...
**Quickest path — one paste.** `supabase/setup_all.sql` is the six migrations
plus the Veloura Lab seed, concatenated in order. Paste the whole file into
**Supabase → SQL Editor → New query** and run it once. You should see seven
`== applied: ... ===` lines and nothing else; routine chatter is silenced so
that anything else which appears is worth reading — especially a warning
about **storage**, which is the one failure that breaks photo uploads later
(see [If photo uploads fail](#if-photo-uploads-fail)). It is generated by
